...
class PagemonitorSpider(scrapy.spiders.Spider):
    ...
    def parse(self, response):
        title = selector.xpath('//h1[@class="product-item-headline"]/text()').extract()
        price = sel.xpath('//section[contains(@class,"product-detail-meta")]//span[@class="price-value"]/text()').extract()
        oriprice = sel.xpath('//section[contains(@class,"product-detail-meta")]//small[@class="price-value-original"]/text()').extract()
